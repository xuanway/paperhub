---
title: "LATPC: Accelerating GPU Address Translation Using Locality-Aware TLB Prefetching and MSHR Compression"
description: "MICRO 2025 · GPU"
tags:
  - "MICRO2025"
  - "GPU"
---

# LATPC: Accelerating GPU Address Translation Using Locality-Aware TLB Prefetching and MSHR Compression

<div class="paper-seo-summary">
<p class="paper-seo-summary__desc">该论文收录于 MICRO 2025，所属 Track: GPU。</p>
<p class="paper-seo-summary__tags">MICRO 2025 · GPU</p>
</div>

**论文链接**：
**作者**：Yeonan Ha, Jiho Park, Hanna Cha (Yonsei Univ.); Jiwon Lee (Yonsei Univ. / Samsung Electronics); Joonsung Kim (Sungkyunkwan Univ.); Won Woo Ro, Youngsok Kim (Yonsei Univ.)
**会议**：MICRO 2025

---

## 一句话总结

> 该工作面向 GPU 场景，围绕 LATPC、Accelerating 相关关键问题 提出系统化优化方案，重点涉及 LATPC、Accelerating、GPU 等关键技术点。

## 方法简述

- 以 LATPC 为核心切入点，构建针对 GPU 工作负载的优化路径。
- 从算法、编译或体系结构层面进行联合设计，减少关键瓶颈。
- 在 GPU 执行与调度路径上引入针对性优化。

## 主要结果

- 提升并行执行效率与资源利用率。
- 目标是降低时延并提升吞吐/可扩展性。
- 方法具备与现有软硬件栈集成的潜力，适用于后续扩展验证。
