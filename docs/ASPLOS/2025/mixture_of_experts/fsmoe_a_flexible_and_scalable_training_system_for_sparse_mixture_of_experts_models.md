---
title: "FSMoE: A Flexible and Scalable Training System for Sparse Mixture-of-Experts Models"
description: "ASPLOS 2025 · Mixture of Experts"
tags:
  - "ASPLOS2025"
  - "Mixture of Experts"
---

# FSMoE: A Flexible and Scalable Training System for Sparse Mixture-of-Experts Models

<div class="paper-seo-summary">
<p class="paper-seo-summary__desc">该论文收录于 ASPLOS 2025，所属 Track: Mixture of Experts。</p>
<p class="paper-seo-summary__tags">ASPLOS 2025 · Mixture of Experts</p>
</div>

**论文链接**：[https://doi.org/10.1145/3669940.3707272](https://doi.org/10.1145/3669940.3707272)
**作者**：Xinglin Pan (The Hong Kong University of Science and Technology (Guangzhou)), Wenxiang Lin (Harbin Institute of Technology, Shenzhen), Lin Zhang (Hong Kong University of Science and Technology), Shaohuai Shi (Harbin Institute of Technology, Shenzhen), Zhenheng Tang (The Hong Kong University of Science and Technology), Rui Wang (The Hong Kong University of Science and Technology (Guangzhou)), Bo Li (Hong Kong University of Science and Technology), Xiaowen Chu (The Hong Kong University of Science and Technology (Guangzhou),Hong Kong University of Science and Technology)
**会议**：ASPLOS 2025

---

## 一句话总结

> 该工作面向 Mixture of Experts 场景，围绕 A Flexible and Scalable Training System for Sparse Mixture-of-Experts Models 提出系统化优化方案，重点涉及 FSMoE、Flexible、Training 等关键技术点。

## 方法简述

- 以 FSMoE 为核心切入点，构建针对 Mixture of Experts 工作负载的优化路径。
- 从算法、编译或体系结构层面进行联合设计，减少关键瓶颈。
- 通过模块化设计保持方案可迁移性与工程可落地性。

## 主要结果

- 目标是降低时延并提升吞吐/可扩展性。
- 在真实系统落地时兼顾实现复杂度与工程可维护性。
- 方法具备与现有软硬件栈集成的潜力，适用于后续扩展验证。
