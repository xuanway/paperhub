---
title: "Protecting Cryptographic Code Against Spectre-RSB"
description: "ASPLOS 2025 · Side Channels"
tags:
  - "ASPLOS2025"
  - "Side Channels"
---

# Protecting Cryptographic Code Against Spectre-RSB

<div class="paper-seo-summary">
<p class="paper-seo-summary__desc">该论文收录于 ASPLOS 2025，所属 Track: Side Channels。</p>
<p class="paper-seo-summary__tags">ASPLOS 2025 · Side Channels</p>
</div>

**论文链接**：[https://doi.org/10.1145/3676641.3716015](https://doi.org/10.1145/3676641.3716015)
**作者**：Santiago Arranz Olmos (MPI-SP), Gilles Barthe (MPI-SP, IMDEA Software Institute), Chitchanok Chuengsatiansup (University of Melbourne), Benjamin Gregoire (Inria), Vincent Laporte (Université de Lorraine, CNRS, Inria, LORIA), Tiago Oliveira (Sandbox AQ), Peter Schwabe (MPI-SP,Radboud University), Yuval Yarom (Ruhr University Bochum), Zhiyuan Zhang (MPI-SP)
**会议**：ASPLOS 2025

---

## 一句话总结

> 该工作面向 Side Channels 场景，围绕 Protecting Cryptographic Code Against Spectre-RSB 提出系统化优化方案，重点涉及 Protecting、Cryptographic、Code 等关键技术点。

## 方法简述

- 以 Protecting Cryptographic Code Against Spectre-RSB 为核心切入点，构建针对 Side Channels 工作负载的优化路径。
- 从算法、编译或体系结构层面进行联合设计，减少关键瓶颈。
- 通过模块化设计保持方案可迁移性与工程可落地性。

## 主要结果

- 在性能、能效或可扩展性指标上预期优于基线方案。
- 在真实系统落地时兼顾实现复杂度与工程可维护性。
- 方法具备与现有软硬件栈集成的潜力，适用于后续扩展验证。
