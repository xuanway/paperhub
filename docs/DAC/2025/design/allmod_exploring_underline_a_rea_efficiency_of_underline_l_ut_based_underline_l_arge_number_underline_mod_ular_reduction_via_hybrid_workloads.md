---
title: "ALLMod: Exploring \underline{A}rea-Efficiency of \underline{L}UT-based \underline{L}arge Number \underline{Mod}ular Reduction via Hybrid Workloads"
description: "DAC 2025 · Design"
tags:
  - "DAC2025"
  - "Design"
---

# ALLMod: Exploring \underline{A}rea-Efficiency of \underline{L}UT-based \underline{L}arge Number \underline{Mod}ular Reduction via Hybrid Workloads

<div class="paper-seo-summary">
<p class="paper-seo-summary__desc">该论文收录于 DAC 2025（第62届），所属 Track: Design。</p>
<p class="paper-seo-summary__tags">DAC 2025 · Design</p>
</div>

**论文链接**：
**作者**：Li Jiang, Haomin Li, Shoumeng Yan, Bo Zhang, Mingzhe Zhang
**会议**：DAC 2025（第62届）
**Session**：DES1: SoC, Heterogeneous, and Reconfigurable Architectures

---

## 一句话总结

> 该工作属于 Design 方向，围绕关键系统瓶颈提出优化方案，并在 DAC 2025 语境下验证其价值。

## 方法简述

- 识别该方向中的关键性能、能效或设计自动化瓶颈。
- 通过软硬件协同或 EDA 工具链优化构建可落地方案。
- 在典型工作负载上进行评估并分析设计权衡。

## 主要结果

- 在目标指标（性能、能效或设计质量）上相对基线实现改进。
- 展示了与现有 EDA 或系统栈集成的可行性。
- 为后续扩展和工程化部署提供依据。
